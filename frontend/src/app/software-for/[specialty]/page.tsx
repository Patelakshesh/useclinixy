...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = specialtiesData[specialtyParam.toLowerCase()];

  if (data) {
    return {
      title: data.seoTitle,
      description: data.seoDescription,
      keywords: [...data.seoKeywords, 'clinixy', 'clinic management software India'],
      alternates: {
      ...
      openGraph: {
        ...
        siteName: 'Clinixy',
        type: 'website',
        images: [{ url: '/opengraph-image.png', width: 1200, height: 630, alt: data.seoTitle }],
      },
    };
  }
...
export default async function SpecialtyPage({ params }: Props) {
  ...
  const specialtyName = specialtyParam.charAt(0).toUpperCase() + specialtyParam.slice(1);
  const defaultData = {
    title: `The smart way to run your ${specialtyParam} practice.`,
    subtitle: `Made for ${specialtyName}`,
    description: `Clinixy gives ${specialtyParam} everything they need to manage patients, schedule appointments, and grow their clinic without the administrative headache.`,
    features: [
  ...
  const content = specialtiesData[specialtyParam.toLowerCase()] || defaultData;
